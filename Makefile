...
H_FILES  = 
MAN1PODS = 
MAN3PODS = Checksums.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/CPAN
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/CPAN
...
TO_INST_PM = Checksums.pm

PM_TO_BLIB = Checksums.pm \
	$(INST_LIB)/CPAN/Checksums.pm
...
ppd:
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Compress-Bzip2" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Compress-Zlib" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Data-Compare" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
